...
fn persist(path: &std::path::Path, file: &VaultFile) {
    let Ok(bytes) = serde_json::to_vec(file) else { return };
    if let Err(e) = atomic_write_private(path, &bytes) {
        eprintln!("3fa: failed to persist vault: {e}");
    }
}
...
fn atomic_write_private(path: &std::path::Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    // Re-assert 0600 in case the temp file pre-existed with looser bits.
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
